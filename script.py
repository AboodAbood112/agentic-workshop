...
import json
trans_str = json.dumps(translations, ensure_ascii=False, indent=2)
content = content.replace("const levels = [", f"const translations = {trans_str};\n\n  const levels = [")

# Update each level in the javascript code
for level_id, data in levels_en.items():
    # We will inject the new fields right before the closing brace `}` of the level object
    pattern = r'(id:\s*' + str(level_id) + r',.*?)(    },)'
    
    learn_en_str = json.dumps(data['learn_en'], ensure_ascii=False)
    take_en_str = json.dumps(data['take_en'], ensure_ascii=False)
    sec_en_str = json.dumps(data['sec_en'], ensure_ascii=False)
    # Using triple quotes or json.dumps for content_en to escape correctly
    content_en_str = json.dumps(data['content_en'], ensure_ascii=False)
    
    injection = f"""      learn_en: {learn_en_str},
      take_en: {take_en_str},
      sec_en: {sec_en_str},
      content_en: {content_en_str},
"""
    content = re.sub(pattern, r'\1' + injection + r'\2', content, flags=re.DOTALL)

# Inject the JavaScript application logic
js_logic_replacement = """
  const sections_ar = [
    "Class 1 — الأساس",
    "Class 1 — الجودة",
    "Class 1 — التركيب",
  ...
  function applyLang() {
    ...
    if (cur) openPanel(cur.id); // re-render panel
    render();
  }
  ...
  function openPanel(id) {
    ...
    // In English, we don't have English HTML files, so we always use content_en
...
content = re.sub(r'  const sections = \[.*?render\(\);\n</script>', js_logic_replacement, content, flags=re.DOTALL)
